$(document).ready(function() {
    ...
    /*----------------------------------------------------------------------*/
    /* CREAMOS FORMULARIO TOPICOS
                /*----------------------------------------------------------------------*/  
    
    var idCategoria = $("#cat").attr("value");
   
    $('#boxSet').bind('click', function() {     
          
       // var nombre = $('.titulo_categoria').attr("id");
       // var titulo = $('.titulo').attr("id");
        var categoria_nombre = $('.titulo_categoria').attr("id");
        //var token = $('.token').attr("value");
        var id_categoria = $('.id_categoria').attr("value");
  
        $('#content').find('textarea.html').wl_Editor();

        if(!$('.b-c').find("#formsss").length){
            
           
            $form = $('<form id="formsss" style="width:600px;"  action="/nadairreal/posteos" method="post" autocomplete="off"></form>');               
            
            $form.append('<fieldset>'
                + '<label>Nuevo Topico</label>'
                + '<section><label for="textarea_auto">Titulo</label>'
                + '<div><input name="titulo" title="Piensa en un buen titulo! :-)"  type="text" id="text_tooltip" value="" required></div>'
                + '</section>'
                + '<input name="categoria"   type="hidden" id="text_tooltip" value="'+ categoria_nombre +'">'
                +'<section>'
                +'<label for="textarea_auto">Comentarios<br><span>(W)hat (y)ou (s)ee (i)s (w)hat (y)ou (g)et</span></label>'
                +'<div>'
                +' <textarea style="max-height:300px;" class="html" id="textarea_WYSIWYG "  title="Una descripcion breve y clara! :-)" data-autogrow="true" name="descripcion" style="overflow: hidden;" required></textarea>'
                +'</div>'
                +'</section>'
                +'<input name="id_categoria"   type="hidden" id="text_tooltip" value="'+ idCategoria +'" >'
               // +'<input name="nombre"   type="hidden" id="text_tooltip" value="'+ nombre +'" >'


                +' <section>'
                +'<div><a id="close" class="btn cc">Cancelar</a><button class="submit green" name="posteo-topico" value="submitbuttonvalue">Enviar</button></div>'

                +' </section>'
                +'</fieldset>')
            $('.b-c').append($form).css('display', 'block').animate({
                top: '12%'
            }, 100); 
            ...
            $content.find('form').wl_Form({
                onSuccess: function(data, status){
                   // alert(data);
                    $(this).html('<div class="succes"><h2>Bien :-)</h2><p>Se subio con  con exito!!!<br/>Mandamos un mail al grupo N.I</p></div>');
                    setTimeout(function() {
                        $('.b-c').animate({
                            top: '-70%'
                        }, 250, function(){ 
    ...
    /*----------------------------------------------------------------------*/
    /* CREAMOS FORMULARIO COMENTARIOS
                /*----------------------------------------------------------------------*/  
    ...
    $('.aa').bind('click', function() {   
        ...
        var idPosteo = $(this).attr("id");
        var nombre = $('.firma-topico').attr("id");
        var titulo = $('.titulo').attr("id");
        var categoria_nombre = $('.categoria-nombre').attr("value");
        var token = $('.token').attr("value");
        var id_categoria = $('.id_categoria').attr("value");
   
        if(!$('.b-'+ idPosteo).find("#formu").length){
            $form = $('<form id="formu" style="width:600px;"  action="/nadairreal/posteos" method="post" autocomplete="off"></form>');               
            $form.append('<fieldset>'
                ...
                +'<label for="textarea_auto">Comentarios<br><span>(W)hat (y)ou (s)ee (i)s (w)hat (y)ou (g)et</span></label>'
                +'<div>'
                +' <textarea id="textarea_auto" data-autogrow="true" name="texto" style="overflow: hidden;" required></textarea>'
                +'</div>'
                +'</section>'
                // +'<input name="categoria""   type="hidden" id="text_tooltip" value="'+ idCategoria +'" >'
                +' <input name="id_posteo" title="A Tooltip"  type="hidden" id="text_tooltip" value="'+ idPosteo +'" >'
                +' <input name="nombre" title="A Tooltip"  type="hidden" id="text_tooltip" value="'+ nombre +'" >'
                +' <input name="titulo" title="A Tooltip"  type="hidden" id="text_tooltip" value="'+ titulo +'" >'
                +' <input name="categoria" title="A Tooltip"  type="hidden" id="text_tooltip" value="'+ categoria_nombre +'" >'
                +' <input name="token" title="A Tooltip"  type="hidden" id="text_tooltip" value="'+ token +'" >'
                +' <input name="id_categoria" title="A Tooltip"  type="hidden" id="text_tooltip" value="'+ id_categoria +'" >'
                +'<section>'
                +'<label>Tipo de respuesta</label>'
                +'<div>'
                +'<input type="radio" id="apples_radio" name="tipo" CHECKED value="1" required><label for="apples_radio">RESPONDER (al último comentario)</label>'
                +'<input type="radio" id="bananas_radio" name="tipo" value="0"><label for="bananas_radio">AGREGAR (Otro comentario al topico)</label>					'
                +'</div>'
                +'</section>'
                +' <section>'
                +'<div><a id="close-'+ idPosteo +'" class="btn close">Cancelar</a><button class="submit green" name="posteo-chat" value="submitbuttonvalue">Enviar</button></div>'

                +' </section>'
                +'</fieldset>');
            
           
            $('.b-'+ idPosteo).append($form).css('display', 'block').animate({
                top: '12%'
            }, 100); 
		...
		/*----------------------------------------------------------------------*/               
            ...
            $('#close-'+ idPosteo).bind('click', function() {
...
/*-----------------------------------------------------------------------------------------------------------------------------*/
    ...
    ////					
    //					
    //                                        $(this).html('<div class="lion"><h2>VIEJO BRBRBRBRBRB</h2><p>Se Posteo con  con exito!!!</p> <p>Hemos enviado un mail al grupo del Nada Irreal para dar aviso de las novedades</p><a  class="reload btn" title="reload">Fin</a></div>');
    //                                       
    //                                        setTimeout(function() {
    //   //  $('#form').load('http://localhost/ciudadela/nadairreal/dashboard/1/3.html .b-c');
    //}, 2000);
    //				},
    //				onError: function(status, error, jqXHR){
    //					$.msg("Callback on Error\nError Status: "+status+"\nError Msg: "+error);
